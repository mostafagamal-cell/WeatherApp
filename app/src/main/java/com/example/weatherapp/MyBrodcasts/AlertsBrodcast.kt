...
class AlertsBrodcast:BroadcastReceiver() {
    override fun onReceive(context: Context?, p1: Intent?) {
        CoroutineScope(Dispatchers.IO).launch {
            ...
            val repo=Repo.getInstance(LocalDataSource(ForecastDataBase.getDatabase(context!!).yourDao()), RemoteDataSource(API))
                val alert = repo.getAlert(id)

               repo.getWeather(alert.city,context.getSharedPreferences(settings,MODE_PRIVATE).getInt(language,consts.en.ordinal)).collect {
